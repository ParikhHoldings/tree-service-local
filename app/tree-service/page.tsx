...
export const metadata: Metadata = {
  title: 'TreeService Companies by City — Find Local Tree Service Near You',
  description: 'Compare top-rated treeService and tree service companies in 50+ US cities. Real prices on mowing, design, tree trimming, and stump grinding install. Licensed, insured, free quotes.',
}
...
export default function TreeServiceDirectoryPage() {
  return (
    <main className="py-12 px-4">
      <div className="max-w-6xl mx-auto">
        <div className="mb-10">
          ...
          <h1 className="text-3xl font-bold mb-3 text-gray-900">TreeService Companies by City</h1>
          <p className="text-gray-600 text-lg max-w-2xl">Find the best local tree service experts in your city. Compare pricing on tree trimming, yard design, tree trimming, and irrigation. Licensed, insured, and rated.</p>
        </div>
        ...
        {Object.entries(citiesByState).sort(([a], [b]) => a.localeCompare(b)).map(([stateAbbr, cities]) => (
          <div key={stateAbbr} className="mb-10">
            <h2 className="text-xl font-bold mb-4 text-gray-900 border-b border-gray-200 pb-2">
              TreeService in {STATE_NAMES[stateAbbr] || stateAbbr}
            </h2>
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
              {cities.map(city => (
                <Link key={city.slug} href={`/tree-service/${city.slug}`}
                  ...
                  <div>
                    <span className="font-medium group-hover:text-green-700 transition-colors">TreeService in {city.city}</span>
